...
DOWNLOAD_DIR = os.path.join(ARTIFACTS_DIR, "raw_downloads")
os.makedirs(DOWNLOAD_DIR, exist_ok=True)

print("Fetching 15 image URLs from DuckDuckGo...")
results = DDGS().images("construction worker hardhat safety vest site working", max_results=15)

downloaded_images = []
for idx, r in enumerate(results):
    if len(downloaded_images) >= 12:
        break
    url = r.get("image")
    if not url: continue
...
for idx, img_path in enumerate(downloaded_images):
    ...
    try:
        # Run inference
        results = model(img_path, imgsz=640, conf=0.20, iou=0.45)
        
        out_name = f"tested_batch_{idx}.jpg"
...
with open(os.path.join(ARTIFACTS_DIR, "batch_report_fragment.md"), "w") as f:
    f.write("\n".join(report_lines))
